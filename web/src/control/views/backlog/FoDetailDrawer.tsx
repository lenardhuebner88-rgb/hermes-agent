  ...
  return (
    <div className="space-y-4">
          ...
          <section className="grid grid-cols-2 gap-2 sm:grid-cols-4">
            <DetailMetric label={de.backlog.status} value={item.status || "-"} tone={metricTone(STATUS_TONE[item.status])} />
            <DetailMetric label={de.backlog.risk} value={item.risk || "-"} tone={metricTone(RISK_TONE[item.risk] ?? "amber")} />
            <DetailMetric label={de.backlog.owner} value={item.owner || "-"} tone={item.owner ? undefined : "warn"} />
            <DetailMetric label={de.backlog.area} value={item.area || "-"} />
          </section>

          <section className="rounded-card border border-line bg-surface-2 px-3 py-2">
            <Eyebrow>{de.backlog.nextAction}</Eyebrow>
            <p className="mt-1 text-body text-ink">{nextActionForFoItem(item, detail)}</p>
          </section>

          <SectionLines title={de.backlog.decisionWhyNow} lines={detail?.decision} fallback={item.excerpt || "Keine explizite Entscheidung im Body gefunden."} />
          <SectionLines title={de.backlog.acceptanceCriteria} lines={detail?.acceptance_criteria} fallback="Keine Akzeptanzkriterien gefunden." />
          <SectionLines title={de.backlog.evidenceProof} lines={detail?.proofs} fallback={item.result || "Kein letzter Beleg gefunden."} />
          <SectionLines title={de.backlog.blockers} lines={detail?.blockers} fallback="Keine Blocker im Body gefunden." />

          <section className="border-t border-line-soft pt-3">
            <Eyebrow>{de.backlog.sourcePathRef}</Eyebrow>
            <dl className="mt-2 space-y-2 text-body">
              <div><dt className="font-display text-micro uppercase tracking-[0.08em] text-ink-3">{de.backlog.sourcePath}</dt><dd className="break-words font-data text-ink">{detail?.source_path || sourceRef(item)}</dd></div>
              <div><dt className="font-display text-micro uppercase tracking-[0.08em] text-ink-3">{de.backlog.sourceRefLabel}</dt><dd className="break-words font-data text-ink">{detail?.source_ref || "git:origin/main"}</dd></div>
            </dl>
          </section>
          ...
          <div className="grid gap-2 sm:grid-cols-2">
            <CopyButton text={brief} label={de.backlog.copyBrief} copiedLabel="Brief kopiert" />
            <CopyButton text={commissionPrompt} label={de.backlog.copyPrompt} copiedLabel={de.backlog.commissionCopied} />
          </div>
