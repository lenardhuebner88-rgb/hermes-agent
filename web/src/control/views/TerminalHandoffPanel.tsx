...
function ResultBox({
  ...
  return <div className={cn("rounded-xl border p-3 text-xs", cls)}>{children}</div>;
}
...
export function TerminalHandoffPanel({ target, getSelection, onClose }: TerminalHandoffPanelProps) {
  ...
  return (
    <div className="fixed inset-0 z-[60] flex items-stretch justify-center bg-black/60 p-0 sm:items-center sm:p-4">
      <div className="flex h-full w-full max-w-3xl flex-col overflow-hidden rounded-none border border-white/10 bg-[#071b1d] shadow-2xl sm:h-auto sm:max-h-[90vh] sm:rounded-2xl">
        <div className="flex items-center justify-between border-b border-white/10 px-4 py-3">
          <div>
            <p className="hc-eyebrow">Terminal → Handoff</p>
            <h2 className="text-sm font-semibold text-white">Auswahl in PlanSpec oder Kanban überführen</h2>
          </div>
          <button
            type="button"
            onClick={onClose}
            aria-label="Handoff schließen"
            className="rounded-md border border-white/10 p-1.5 text-white/65 hover:bg-white/10"
          >
            <X className="h-4 w-4" />
          </button>
        </div>

        <div className="flex-1 space-y-3 overflow-auto p-4 text-sm text-white/80">
          ...
          <div className="grid gap-2 rounded-xl border border-white/10 bg-black/20 p-3">
            <div className="flex flex-wrap items-center gap-3 text-xs">
              ...
              <label className="flex items-center gap-1.5">
                ...
                <input
                  ...
                  className="w-16 rounded-md border border-white/10 bg-black/30 px-1.5 py-0.5 text-white"
                />
                Zeilen
              </label>
              <button
            ...
            {captured && (
              <pre className="max-h-28 overflow-auto rounded-md border border-white/10 bg-black/40 p-2 text-[11px] text-white/70">
          ...
          <div className="grid gap-2">
            ...
            {mode === "planspec" && (
              <label className="grid gap-1 text-xs">
                <span className="text-white/60">live_test_depth</span>
                <select
                  value={liveTestDepth}
                ...
                >
                  {LIVE_TEST_DEPTHS.map((d) => (
                    <option key={d} value={d} className="bg-[#071b1d]">
                      {d}
                    </option>
                  ))}
                </select>
          ...
          {mode === "planspec" ? (
            <div className="grid gap-2">
              <div className="flex items-center justify-between">
                <span className="text-xs text-white/60">PlanSpec-Draft (editierbar)</span>
                <button
                  type="button"
                  onClick={rebuildDraft}
                  disabled={!captured}
                  className="rounded-md border border-white/10 px-2 py-1 text-[11px] text-white/70 hover:bg-white/10 disabled:opacity-40"
                >
              ...
              <textarea
                ...
                className="w-full rounded-md border border-white/10 bg-black/40 p-2 font-mono text-[11px] text-white/85"
              />
              ...
              {ingestResult && (
                <ResultBox tone="ok">
                  ...
                  <div>
                    Chain / Root-Task: <span className="font-mono text-white">{ingestResult.root_task_id}</span>
                  </div>
                  <div>
                    Subtask-IDs:{" "}
            ...
            <div className="grid gap-2">
              <span className="text-xs text-white/60">Triage-Aufgabentext (editierbar)</span>
              <textarea
                value={taskBody}
                ...
                className="w-full rounded-md border border-white/10 bg-black/40 p-2 text-[12px] text-white/85"
              />
              <button
                type="button"
                onClick={() => void doCreateTask()}
                disabled={busy !== null || !hasText}
                className="w-fit rounded-lg border border-cyan-300/50 bg-cyan-300/10 px-3 py-1.5 text-xs text-cyan-100 hover:bg-cyan-300/20 disabled:opacity-50"
              >
              ...
              {taskResult && (
                <ResultBox tone="ok">
                  ...
                  <div>
                    Task-ID: <span className="font-mono text-white">{taskResult.task.id}</span>
                  </div>
                  <div className="mt-1 text-white/60">
          ...
          <div className="grid gap-2 rounded-xl border border-white/10 bg-black/20 p-3">
            <div className="flex items-center justify-between">
              <span className="text-xs font-semibold text-white/75">Optionale Dispatch-Vorschau</span>
              <button
                type="button"
                onClick={() => void doDispatchDryRun()}
                disabled={busy !== null}
                className="inline-flex items-center gap-1.5 rounded-md border border-white/15 px-2.5 py-1 text-[11px] text-white/80 hover:bg-white/10 disabled:opacity-50"
              >
                ...
                {dispatchPreview.spawned.length > 0 && (
                  <ul className="ml-4 list-disc space-y-0.5">
                    {dispatchPreview.spawned.map(([id, assignee], i) => (
                      <li key={i}>
                        <span className="font-mono text-white">{id}</span> → {assignee || "—"}
                      </li>
                    ))}
                  </ul>
                )}
          ...
          {error && (
            <div className="flex items-start gap-1.5 rounded-lg border border-red-300/30 bg-red-300/10 p-2 text-xs text-red-100">
