...
export function ProposalCard({ proposal, density, busy, onApply, onSkip }: Props) {
  ...
  const isDone = proposal.status !== "proposed";
  return (
    <article className={cn("hc-card space-y-4 p-4", density === "compact" && "p-3")}>
      ...
      {isCode ? (
        <ToneCallout tone="amber"><ShieldAlert className="mr-2 inline h-4 w-4" />{de.autoresearch.codeGate}</ToneCallout>
      ) : null}
      ...
      {isDone ? (
        ...
        <div className="flex flex-col gap-2 sm:flex-row sm:justify-end">
          ...
          <Button className="hc-hit" onClick={() => onApply(proposal)} disabled={busy || isCode} prefix={busy ? <Spinner /> : <Check className="h-4 w-4" />}>
            {de.autoresearch.apply}
          </Button>
        </div>
      )}
    </article>
