...
const WindowedRollupWorkerSchema = ChainCostsLaneSchema.extend({
  ...
  cost_usd_equivalent_applicable: z.boolean().optional(),
  cost_usd_equivalent_state: z.enum(["not_applicable", "complete", "partial", "unknown"]).optional(),
...
export const WindowedRollupResponseSchema = z.object({
  ...
  returned_roots: z.coerce.number().catch(0).optional(),
  truncated: z.boolean().catch(false).optional(),
  totals: z.object({
    root_count: z.coerce.number().catch(0),
    cost_usd: z.coerce.number().catch(0),
    cost_usd_equivalent: z.coerce.number().catch(0),
    cost_usd_known_roots: z.coerce.number().catch(0),
    cost_usd_equivalent_known_roots: z.coerce.number().catch(0),
    cost_usd_equivalent_applicable_roots: z.coerce.number().catch(0).optional(),
    cost_usd_equivalent_state: z.enum(["not_applicable", "complete", "partial", "unknown"]).optional(),
    unknown_run_count: z.coerce.number().catch(0),
  }).catch({
    root_count: 0,
    ...
    cost_usd_equivalent_state: "not_applicable" as const,
    unknown_run_count: 0,
  }).optional(),
