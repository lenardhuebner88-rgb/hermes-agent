...
def estimate_call_tokens(messages: Iterable[Mapping[str, Any]] | None, max_tokens: int) -> int:
    """Conservative reservation: ~chars/4 prompt tokens plus the full output
    allowance. Never below 1 so a real call can never reserve zero."""
    chars = 0
    ...
    return max(1, chars // 4 + max(0, int(max_tokens or 0)))
...
class DailyLedger:
    ...
    def check_call(self, estimated_tokens: int) -> None:
        """Refuse the next call before it happens (call 31+ or crossing the
        daily token limit)."""
        calls = self.calls_today()
        if calls + 1 > self.config.daily_model_call_limit:
            raise BudgetExhausted(
                "autoresearch daily budget exhausted: "
                f"{calls} model calls today >= limit {self.config.daily_model_call_limit}"
            )
    ...
    def record_call(
        ...
        *,
        ...
        """Persist one model call. Measured provider usage replaces the
        reservation; missing usage keeps the conservative estimate and marks
        the entry ``estimated`` — never a measured zero."""
        estimate = max(1, int(estimated_tokens or 0))
...
def quota_block_reason(decision: QuotaDecision, model: str | None) -> str | None:
    ...
    if decision.stop_session:
        return f"quota skip: {decision.reason}"
    if is_expensive_model(model) and not decision.allow_expensive:
...
def guarded_llm_call(
    ...
    **kwargs: Any,
    ...
    """Reserve → call → reconcile. Raises :class:`BudgetExhausted` before the
    provider is contacted when the shared daily ledger is spent."""
    led = ledger or DailyLedger()
    estimate = estimate_call_tokens(messages, max_tokens)
    led.check_call(estimate)
    ...
    entry = led.record_call(lane=lane, model=model, estimated_tokens=estimate, usage=usage)
    return resp, entry
