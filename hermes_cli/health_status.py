...
# Probes must not share default executor with PTY pumps (starvation)
_PROBE_EXECUTOR = concurrent.futures.ThreadPoolExecutor(
    max_workers=4, thread_name_prefix="health-probe"
)


async def _run_probe(fn: Any, /, *args: Any, **kwargs: Any) -> Any:
    """Run a sync probe on the private pool (not the default asyncio executor)."""
    loop = asyncio.get_running_loop()
    if args or kwargs:
        return await loop.run_in_executor(
            _PROBE_EXECUTOR, functools.partial(fn, *args, **kwargs)
        )
...
async def _probe_gateway_status() -> dict[str, Any]:
    ...
    try:
        ...
        pid = await _run_probe(get_running_pid)
        if pid is not None:
            return _status_dict("healthy", "gateway running", latency_ms=_latency_ms(start))
        alive, body = await _run_probe(_probe_gateway_health)
...
async def _probe_autoresearch_status() -> dict[str, Any]:
    ...
    try:
        ...
        runner_status = await _run_probe(read_runner_status)
    except Exception as exc:
...
async def _probe_kanban_db_status() -> dict[str, Any]:
    return await _run_probe(_probe_kanban_db_sync)
...
async def _probe_kanban_dispatcher_status() -> dict[str, Any]:
    return await _run_probe(_probe_kanban_dispatcher_sync)
