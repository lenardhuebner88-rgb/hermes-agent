...
def _is_sensitive_path(path: Path) -> bool:
    ...
    The same predicate also protects upload/mkdir/delete so the dashboard
    cannot overwrite or remove a credential store when its managed root is
    pointed at HERMES_HOME.
    """
...
def _resolve_managed_path(
    ...
    *,
    ...
    if for_write and candidate.is_symlink():
        raise HTTPException(status_code=403, detail="Write target must not be a symlink")
...
_ACCOUNT_USAGE_PROVIDERS: Tuple[str, ...] = (
    "anthropic",
    "openai-codex",
    "kimi",
    "xai",
...
def _account_usage_payload(snapshot: Any, provider: str) -> Dict[str, Any]:
    ...
    return {
        ...
        "signal_at": _isoformat_or_none(getattr(snapshot, "signal_at", None)),
...
def _submit_anthropic_pkce(
    ...
    if result is None:
        with _oauth_sessions_lock:
            sess["status"] = "error"
            sess["error_message"] = scrub_detail(f"Token exchange failed: {last_exc}") or "Token exchange failed"
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=safe_detail(exc, "Hub preview failed", log=_log),
        )
    if result is None:
...
async def scan_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    try:
        ...
        raise HTTPException(
            status_code=502,
            detail=safe_detail(exc, "Hub scan failed", log=_log),
        )
    if result is None:
        raise HTTPException(status_code=404, detail=f"Skill not found: {ident}")
    return result
