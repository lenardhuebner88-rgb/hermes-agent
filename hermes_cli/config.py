...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        #
        # Fork default: 180s graceful drain (upstream defaults to 0 =
        # interrupt immediately). 180s is calibrated for realistic in-flight
        # agent turns: a typical coding conversation mid-reasoning runs
        # 60–150s per call, so a shorter budget routinely interrupted
        # legitimate work on /restart. Keep it well under systemd's
        # TimeoutStopSec; raise further in config.yaml if you run
        # very-long-reasoning models.
        #
        # Upstream's counter-argument (kept here so the divergence stays a
        # decision, not an accident): a drain window can never outlast an
        # unbounded task, and one shorter than TimeoutStopSec invites a
        # SIGKILL-mid-cleanup race leaving a stale lock. That risk is accepted
        # deliberately — 180s is well under our TimeoutStopSec.
    ...
    "kanban": {
        ...
        "auto_retry_blocked_backoff_seconds": 300,
