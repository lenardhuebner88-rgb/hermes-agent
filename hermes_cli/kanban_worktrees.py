...
def _review_snapshot_min_free_bytes() -> int:
    raw = os.environ.get("HERMES_KANBAN_REVIEW_SNAPSHOT_MIN_FREE_BYTES", "536870912")
    try:
...
def _review_snapshot_repo_root(source_workspace: Path) -> Path:
    common_dir = Path(
        _git(source_workspace, "rev-parse", "--path-format=absolute", "--git-common-dir").strip()
    ).resolve()
    if common_dir.name == ".git":
        return common_dir.parent
    return repo_root_for(source_workspace)


def _set_review_snapshot_read_only(path: Path, *, read_only: bool) -> None:
    if not path.exists():
        return
    entries = [path]
    entries.extend(path.rglob("*"))
    ordered = entries if read_only else list(reversed(entries))
    for entry in ordered:
        if entry.is_symlink():
            continue
        try:
            mode = entry.stat().st_mode
            entry.chmod(mode & ~0o222 if read_only else mode | 0o200)
        except FileNotFoundError:
            continue
...
def hygiene_sweep_review_snapshots(
    ...
    *,
    ...
    base = repo_root / ".worktrees" / "kanban-review"
    result: dict[str, Any] = {
        "scanned": 0, "removed": [], "skipped": [], "errors": [],
        "dry_run": bool(dry_run), "base": str(base),
    }
    if not base.is_dir():
        return result
    observed = time.time() if now is None else float(now)
    for entry in sorted(base.iterdir(), key=lambda item: item.name):
        result["scanned"] += 1
        try:
            age = max(0.0, observed - entry.stat().st_mtime)
        except OSError as exc:
            result["errors"].append(f"{entry}: {exc}")
            continue
        if age < max(0, int(max_age_seconds)):
            result["skipped"].append({"path": str(entry), "reason": "too_fresh"})
            continue
        if dry_run:
            result["removed"].append({"path": str(entry), "dry_run": True})
            continue
        try:
            cleanup_review_snapshot(repo_root=repo_root, workspace_path=entry)
            result["removed"].append({"path": str(entry), "dry_run": False})
        except Exception as exc:
            result["errors"].append(f"{entry}: {exc}")
    return result
...
def provision_review_snapshot(
    *,
    ...
    repo_root = _review_snapshot_repo_root(source_workspace)
    candidate = _git(repo_root, "rev-parse", "--verify", f"{candidate_commit}^{{commit}}").strip()
    base = None
    if base_commit:
        base = _git(repo_root, "rev-parse", "--verify", f"{base_commit}^{{commit}}").strip()
    hygiene_sweep_review_snapshots(repo_root)
    free_bytes = shutil.disk_usage(repo_root).free
    required_bytes = _review_snapshot_min_free_bytes()
    if free_bytes < required_bytes:
        raise RuntimeError(
            "review snapshot disk preflight failed: "
            f"free={free_bytes} required={required_bytes} repo={repo_root}"
        )
    token = re.sub(r"[^A-Za-z0-9_.-]+", "-", task_id).strip("-._") or "task"
    snapshot_root = repo_root / ".worktrees" / "kanban-review"
    snapshot_root.mkdir(parents=True, exist_ok=True)
    workspace_path = snapshot_root / f"{token}-r{int(run_id)}-{candidate[:12]}"
...
def cleanup_review_snapshot(*, repo_root: Path, workspace_path: Path) -> None:
    repo_root = Path(repo_root).resolve()
    workspace_path = Path(workspace_path).resolve()
    allowed_root = (repo_root / ".worktrees" / "kanban-review").resolve()
    try:
    ...
    if result.returncode != 0 and workspace_path.exists():
        shutil.rmtree(workspace_path, ignore_errors=False)
    _git(repo_root, "worktree", "prune")
    try:
        allowed_root.rmdir()
        allowed_root.parent.rmdir()
    except OSError:
        pass
