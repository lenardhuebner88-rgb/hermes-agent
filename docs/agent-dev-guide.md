# Hermes Agent - Detailed Development Guide

Detailed reference for AI coding assistants and developers working on the hermes-agent codebase.
The compact startup context lives in [`../AGENTS.md`](../AGENTS.md); keep the startup file small and move subsystem detail here.
...
1. **Check-IN** before the first edit — run
   `python3 /home/piet/vault/_agents/_shared/scripts/coordination-open-sessions.py` (parses
   frontmatter only — no `grep`). If an open `touching:` overlaps your planned paths, **stop**
...
- **10-minute inactivity timeout** on cron sessions (default; override via
  `HERMES_CRON_TIMEOUT`, `0` = unlimited) — a session with no tool/API/stream
  activity for that long is killed, but an actively working session can run
  for hours.
