...
|---|---|---|---|---|
| creation → triage | [`create_task`](../../hermes_cli/kanban_db.py#L5405) | CLI / automation | invalid initial fields, missing parents, role/workspace contract | no row is created; caller gets a validation error |
| creation → todo | [`create_task`](../../hermes_cli/kanban_db.py#L5405) | CLI / automation | missing parent IDs | new card waits in `todo`; an unfinished parent is visible |
| creation → ready | [`create_task`](../../hermes_cli/kanban_db.py#L5405) | CLI / automation | unfinished parents route to `todo` instead | card is `todo`, not `ready` |
| creation → blocked | [`create_task`](../../hermes_cli/kanban_db.py#L5405) | CLI / automation | only the explicit blocked initial mode is accepted | card is born as an operator park |
| triage → todo | [`specify_triage_task`](../../hermes_cli/kanban_db.py#L19256) | CLI / sweep | row must still be in `triage` | specify returns false; status did not change |
| todo → ready | [`recompute_ready`](../../hermes_cli/kanban_db.py#L11765) | dispatcher daemon / completion sweep | parents, due time, typed wait, sticky block, active escalation, failure limit | card remains `todo` or `blocked`; no `promoted` event |
| todo\|blocked → ready | [`promote_task`](../../hermes_cli/kanban_db.py#L18412) | CLI | typed wait or unfinished parents; blocked tasks cannot force past parents | command reports the refusing parent/wait |
| ready → todo | [`claim_task`](../../hermes_cli/kanban_db.py#L11905) | dispatcher daemon / CLI claimer | active wait or any parent not `done` | `claim_rejected` event; reason is `active_wait` or `parents_not_done` |
| ready → running | [`claim_task`](../../hermes_cli/kanban_db.py#L11905) | dispatcher daemon / CLI claimer | code contract, wait, parents, claim CAS | card stays `ready`; no current run is opened |
| review → running | [`claim_review_task`](../../hermes_cli/kanban_db.py#L12144) | review gate / dispatcher daemon | already claimed, wrong status, unavailable review profile before claim | card stays `review`; review hold event identifies the target |
| running\|ready → blocked | [`block_task`](../../hermes_cli/kanban_db.py#L18071) | worker / CLI / review gate | stale run ID, wrong status, invalid review-origin contract | block returns false; rejection event explains the mismatch |
| running\|ready → todo | [`block_task`](../../hermes_cli/kanban_db.py#L18071) | worker | a valid unsatisfied dependency wait deliberately parks on `todo` | `wait_registered` plus a `blocked` event whose status is `todo` |
| running\|ready → triage | [`block_task`](../../hermes_cli/kanban_db.py#L18071) | worker / sweep | same non-review block kind must reach [`BLOCK_RECURRENCE_LIMIT`](../../hermes_cli/kanban_db.py#L181) | `block_loop_detected` event; card is in `triage` |
| running\|ready\|blocked → review | [`_submit_for_review`](../../hermes_cli/kanban_db.py#L15551) | worker / review gate | workflow identity, worker gate, expected run CAS | task blocks on identity failure or completion raises a worker-gate error |
| running → blocked | [`hold_task`](../../hermes_cli/kanban_db.py#L12831) | CLI | task must still be running | hold returns false; worker/status won the race |
| triage\|todo\|scheduled\|ready\|running\|review → blocked | [`cancel_chain`](../../hermes_cli/kanban_db.py#L12934) | CLI | final/already-blocked members are skipped | result lists members under `skipped` |
| running\|ready\|blocked → ready | [`_advance_workflow_step`](../../hermes_cli/kanban_db.py#L15915) | worker / workflow gate | no valid next step, stale run ID, wrong status | task completes normally or update returns false |
| running → review | [`_maybe_advance_review_chain`](../../hermes_cli/kanban_db.py#L16079) | review gate | verdict must be approved, workflow identity valid, another stage exists | identity failure blocks; final stage proceeds to `done` |
| running\|ready\|blocked → done | [`complete_task`](../../hermes_cli/kanban_db.py#L16395) | worker / CLI / review gate | hallucinated cards, worker/review gate, stale run, integration park | exception/event or card moves to `review`/`blocked` instead |
| running → ready\|review | [`release_stale_claims`](../../hermes_cli/kanban_db.py#L12553) | dispatcher daemon | live local PID plus fresh heartbeat extends claim; surviving process prevents release | `claim_extended` or `reclaim_deferred`; card remains `running` |
| running\|ready\|blocked → ready | [`reclaim_task`](../../hermes_cli/kanban_db.py#L12736) | CLI | a worker process group that survives termination | `reclaim_deferred`; claim stays owned |
| blocked\|scheduled → todo\|ready | [`unblock_task`](../../hermes_cli/kanban_db.py#L19117) | CLI / automation | typed wait unless audited override; unfinished parents route to `todo` | unblock returns false or succeeds into `todo` |
| todo\|ready\|running\|blocked → scheduled | [`schedule_task`](../../hermes_cli/kanban_db.py#L21134) | CLI / cron / worker | active typed wait, stale expected run ID | schedule returns false; previous status remains |
| any non-archived → archived | [`archive_task`](../../hermes_cli/kanban_db.py#L20313) | CLI / sweep | dependent wait, changed ownership generation, surviving worker | archive refuses; wait conflict or `reclaim_deferred` is visible. Operator archives (CLI + dashboard, `retrigger_integration=True`) also re-run chain integration after a successful archive — synchronous gate+merge in the caller, see `maybe_retrigger_integration_after_archive` |

## Dispatch path: tick to worker process

1. [`dispatch_once`](../../hermes_cli/kanban_db.py#L30075) resolves the board DB
...
2. [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L30166) reaps zombies
   and pending continuations, refreshes Claude-CLI heartbeats through
   [`heartbeat_live_claude_cli_workers`](../../hermes_cli/kanban_db.py#L22325),
   reclaims stale/dead/timed-out runs, optionally retries settled blocks, then
   calls [`recompute_ready`](../../hermes_cli/kanban_db.py#L11765).
3. It selects unclaimed `ready` rows by priority and age. Before claim it applies
   ...
   contract, and [`check_respawn_guard`](../../hermes_cli/kanban_db.py#L28387).
   Advisory holds leave the row `ready`; [`summarize_dispatch_holds`](../../hermes_cli/kanban_db.py#L28627)
   groups the operator-visible buckets.
4. [`claim_task`](../../hermes_cli/kanban_db.py#L11905) performs the
   `ready → running` CAS, stamps claim expiry and immutable route identity, and
   creates the run. Review rows take the parallel
   [`claim_review_task`](../../hermes_cli/kanban_db.py#L12144) path.
5. [`_resolve_dispatch_workspace`](../../hermes_cli/kanban_db.py#L21024)
   chooses managed worktree provisioning or existing-workspace resolution.
   [`resolve_workspace`](../../hermes_cli/kanban_db.py#L20919) defines the
...
6. A writable shared chain worktree acquires a writer lease. Then
   [`_default_spawn`](../../hermes_cli/kanban_db.py#L32586) freezes the worker
   environment and launch route, and [`_launch_worker_process`](../../hermes_cli/kanban_db.py#L31943)
...
1. [`claim_task`](../../hermes_cli/kanban_db.py#L11905) (and
   [`claim_review_task`](../../hermes_cli/kanban_db.py#L12144)) call
   [`_spawn_identity_metadata`](../../hermes_cli/kanban_db.py#L8089), which
   resolves model and provider and persists them on the run.
2. [`_default_spawn`](../../hermes_cli/kanban_db.py#L32586) reads that stamp back
   via [`_persisted_spawn_identity`](../../hermes_cli/kanban_db.py#L32379) and
...
[`_resolve_model_override`](../../hermes_cli/kanban_db.py#L7952) is the single
resolver, and
...
[`_handle_incompatible_model_override`](../../hermes_cli/kanban_db.py#L8066),
...
> [`_default_spawn`](../../hermes_cli/kanban_db.py#L32586) is exactly that — it
...
[`build_worker_context`](../../hermes_cli/kanban_db.py#L34061) returns the
canonical phase-aware bounded brief built by
[`render_worker_brief_for_task`](../../hermes_cli/kanban_db.py#L34022). It
...
[`_default_spawn`](../../hermes_cli/kanban_db.py#L32586) additionally pins the
...
[`heartbeat_worker`](../../hermes_cli/kanban_db.py#L22141); Claude-CLI lanes are
bridged by [`heartbeat_live_claude_cli_workers`](../../hermes_cli/kanban_db.py#L22325).
The daemon publishes board counts and tick health through
[`write_kanban_dispatcher_heartbeat`](../../hermes_cli/kanban_db.py#L28332).
...
[`complete_task`](../../hermes_cli/kanban_db.py#L16395) is a funnel, not a
single write. The order matters and is load-bearing:

1. **Workflow step routing** — a non-final workflow step re-queues instead of
   completing.
2. **Review gate** — [`_review_gate_should_apply`](../../hermes_cli/kanban_db.py#L14765):
...
3. **Token levers** — [`_deterministic_review_skip`](../../hermes_cli/kanban_db.py#L15371)
   and [`_tip_defer_review`](../../hermes_cli/kanban_db.py#L15413) may skip the
   LLM review; otherwise [`_submit_for_review`](../../hermes_cli/kanban_db.py#L15551)
...
5. **Stage advance** — [`_maybe_advance_review_chain`](../../hermes_cli/kanban_db.py#L16079)
...
[`integrate_chain`](../../hermes_cli/kanban_worktrees.py#L6283) is *the* single
...
|---|---|---|---|
| 0 | live-checkout precheck | [`_integrate_precheck_live`](../../hermes_cli/kanban_worktrees.py#L5909) | `MERGE_HEAD`/rebase in progress, or checked-out branch ≠ frozen merge target |
| 1 | artifact preservation | [`_preserve_or_park_chain_artifacts`](../../hermes_cli/kanban_worktrees.py#L5946) | chain worktree dirty with non-preservable files |
| 2 | nothing-to-merge | [`_integrate_empty_or_already_merged`](../../hermes_cli/kanban_worktrees.py#L5999) | handles `ahead == 0`: already-integrated, or replays a previously reverted merge |
| 3 | dirty overlap | inline in `integrate_chain` | a foreign dirty file in the live checkout overlaps the branch diff |
| 4 | rebase onto target | [`_integrate_rebase_branch`](../../hermes_cli/kanban_worktrees.py#L6169) | conflict → `rebase_conflict` (routed back to the coder, **not** a park) |
| 5 | merge + post-merge gate | [`_integrate_merge_and_gate`](../../hermes_cli/kanban_worktrees.py#L6219) | merge conflict → `merge --abort`; red gate → `revert -m 1` + park |

The post-merge gate runs at the exact merge commit inside a detached validation
worktree — [`_run_gate_in_validation_worktree`](../../hermes_cli/kanban_worktrees.py#L5550)
...
[`fo_integration_gate`](../../hermes_cli/kanban_worktrees.py#L5843) for the FO
repo, else [`default_quick_gate`](../../hermes_cli/kanban_worktrees.py#L5816).
...
A park becomes [`_park_integration`](../../hermes_cli/kanban_db.py#L17735),
...
[`maybe_integrate_on_complete`](../../hermes_cli/kanban_worktrees.py#L7850) only
integrates when this completion closes the **last open task** of a provisioned
chain. [`_find_open_chain_sibling`](../../hermes_cli/kanban_worktrees.py#L7008)
...
**Tier resolution** —
[`_effective_review_tier`](../../hermes_cli/kanban_db.py#L13423) treats the
...
[`_review_tier_downgrade_acked`](../../hermes_cli/kanban_db.py#L13476). The
classifier itself,
...
requires *positive* evidence: [`_run_worker_gate`](../../hermes_cli/kanban_db.py#L15278)
...
[`_capture_review_diff_snapshot`](../../hermes_cli/kanban_db.py#L15054) against
...
| Respawn cooldown/duplicate-work guard | recent rate limit/transient retry/auth failure/success, active PR, or invalid code contract | [`check_respawn_guard`](../../hermes_cli/kanban_db.py#L28387) | card remains `ready`; deduped `respawn_guarded` event names the reason | wait for cooldown, correct auth/contract, close or deliberately requeue after successful work, resolve the PR |
| G1 cumulative input-token runaway | all-run input sum exceeds configured per-task cap; one actionable review extension may be allowed | [`_park_budget_runaway`](../../hermes_cli/kanban_db.py#L25690) | card becomes capacity-blocked; `budget_runaway_parked` and operator-escalation events include sum/cap/run count | inspect the runaway, then operator unblocks/reassigns/closes; raising the cap alone does not change the parked state |
| Dispatch holds | repo, chain-worktree, writer lease, daily budget, role mismatch, or per-profile concurrency is occupied | [`summarize_dispatch_holds`](../../hermes_cli/kanban_db.py#L28627) | card stays `ready`; tick bucket/event names `repo_serialized`, `chain_worktree_serialized`, `worktree_writer_active`, `budget_held`, `role_fit_held`, or profile cap | let holder finish/reclaim, repair stale writer ownership, adjust routing/cap, or remove the conflicting work |
| Review ping-pong breaker | repeated review-origin `REQUEST_CHANGES` reaches configured maximum rounds | [`block_task`](../../hermes_cli/kanban_db.py#L18071) | task is `blocked` as `needs_input`; reason starts `review ping-pong breaker`; operator escalation class is `review_pingpong` | operator resolves findings and explicitly unblocks/respecs/reassigns; it does not auto-retry |
| Parent/wait gate | parent is not `done`, due time is future, or typed wait remains unsatisfied/invalid | [`recompute_ready`](../../hermes_cli/kanban_db.py#L11765) | task remains `todo`/`blocked`; claim may emit `claim_rejected` | finish parent, wait for due/event, or use the audited wait override |
| Global/profile concurrency | live running count reaches board, spawn, or profile cap | [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L30166) | no spawn; profile-capped tasks appear in the hold result, global cap returns early | allow running tasks to terminate/reclaim or change the configured cap |
| Nonspawnable assignee | assigned name is not an on-disk Hermes profile | [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L30166) | `nonspawnable` event; unknown lanes also emit one operator escalation | assign a real profile or provision the intended profile; terminal pull-only lanes remain intentionally held |
| Workspace provisioning/base drift | invalid path, worktree lock/timeout, rebase conflict, or reused base differs | [`_resolve_dispatch_workspace`](../../hermes_cli/kanban_db.py#L21024) | claimed run is requeued/blocked; events include spawn retry/failure or `worker_base_rejected` | fix path/git state; transient timeouts back off; conflicts use the bounded fixer path or operator repair |
| Claim/heartbeat expiry | TTL expires and no sufficiently fresh observable progress exists | [`release_stale_claims`](../../hermes_cli/kanban_db.py#L12553) | `reclaimed`, `claim_extended`, or `reclaim_deferred` event; status is `ready`, `review`, or still `running` | healthy workers heartbeat; terminate/reclaim a wedged process; never clear ownership while its process survives |
| Worker exits without terminal lifecycle call | subprocess exits zero while task is still running | [`detect_crashed_workers`](../../hermes_cli/kanban_db.py#L22993) | `protocol_violation` or `deliverable_posted_not_completed`; bounded repeats end in `gave_up` | recover posted evidence or rerun with the required complete/block call; operator unblocks after breaker trip |
| Spawn/config failure breaker | model route, executable, workspace, or repeated spawn fails | [`_record_spawn_failure`](../../hermes_cli/kanban_db.py#L24708) | failure counter and spawn events accumulate; terminal attempt becomes blocked/auto-blocked | repair deterministic config; allow bounded transient retry, then explicitly unblock after correction |
| Sticky worker/operator block | latest block or active escalation requires a decision | [`_has_sticky_block`](../../hermes_cli/kanban_db.py#L11580) | card stays `blocked` even when every parent is done | [`unblock_task`](../../hermes_cli/kanban_db.py#L19117) after resolving the stated cause |
| Integration park | a precheck, the merge, or the post-merge gate failed | [`integrate_chain`](../../hermes_cli/kanban_worktrees.py#L6283) → [`_park_integration`](../../hermes_cli/kanban_db.py#L17735) | `integration_parked` event; closing run outcome is `integration_parked`, task `blocked` | fix the stated cause, then unblock; a red gate means the merge was already reverted |
| Rebase conflict | chain branch does not replay onto the live target | [`_integrate_rebase_branch`](../../hermes_cli/kanban_worktrees.py#L6169) | `integration_rebase_conflict`; rebase aborted, worktree back at its committed state | routed back to the coder as fixer work, deliberately NOT an operator park |
...
[`_record_task_failure`](../../hermes_cli/kanban_db.py#L24299);
[`_record_spawn_failure`](../../hermes_cli/kanban_db.py#L24708) delegates to it.
...
The post-merge [`default_quick_gate`](../../hermes_cli/kanban_worktrees.py#L5816)
enforces the same hold. Its compatibility mapper,
[`_affected_pytest_modules`](../../hermes_cli/kanban_worktrees.py#L4912), and
...
[`_park_integration`](../../hermes_cli/kanban_db.py#L17735) blocks via an UPDATE
...
[`create_task`](../../hermes_cli/kanban_db.py#L5405) because that is the common
...
| 5959 | [Links](../../hermes_cli/kanban_db.py#L6461) |
| 6316 | [Comments & events](../../hermes_cli/kanban_db.py#L6818) |
| 6403 | [Attachments](../../hermes_cli/kanban_db.py#L6905) |
| 10195 | [Dependency resolution (todo -> ready)](../../hermes_cli/kanban_db.py#L10795) |
| 11301 | [Claim / complete / block](../../hermes_cli/kanban_db.py#L11901) |
| 12645 | [Review gate (Phase 2: independent verification before 'done')](../../hermes_cli/kanban_db.py#L13245) |
| 16100 | [Workspace / tmux cleanup](../../hermes_cli/kanban_db.py#L17009) |
| 16579 | [First-use tip for scratch workspaces](../../hermes_cli/kanban_db.py#L17490) |
| 19637 | [Workspace resolution](../../hermes_cli/kanban_db.py#L20609) |
| 20188 | [Dispatcher (one-shot pass)](../../hermes_cli/kanban_db.py#L21201) |
| 20218 | [Respawn guard constants](../../hermes_cli/kanban_db.py#L21231) |
| 24185 | [G1: per-task cumulative input-token runaway guard](../../hermes_cli/kanban_db.py#L25484) |
| 27527 | [OpenClaw cross-system dispatch (Mission-Control via HMAC-signed envelopes)](../../hermes_cli/kanban_db.py#L28842) |
| 27764 | [B1a — tree-wide inventory/hygiene must not land on research/premium LLM loops](../../hermes_cli/kanban_db.py#L29079) |
| 31557 | [Long-lived dispatcher daemon](../../hermes_cli/kanban_db.py#L32907) |
| 31631 | [Worker context builder (what a spawned worker sees)](../../hermes_cli/kanban_db.py#L32981) |
| 32263 | [Scope-contract template expansion (PlanSpec B)](../../hermes_cli/kanban_db.py#L33613) |
| 32705 | [Stats + SLA helpers](../../hermes_cli/kanban_db.py#L34075) |
| 35429 | [Epics (N-E3) — durable goals spanning multiple task trees](../../hermes_cli/kanban_db.py#L36815) |
| 35603 | [Disposition Ledger (FRD-S1) — additive; no wiring into completion path yet](../../hermes_cli/kanban_db.py#L36989) |
| 36457 | [Lanes (night-sprint F1) — switchable profile→(runtime, model) presets](../../hermes_cli/kanban_db.py#L37903) |
| 36887 | [Notification subscriptions (used by the gateway kanban-notifier)](../../hermes_cli/kanban_db.py#L38341) |
| 37006 | [Browser Web Push subscriptions (used by the control dashboard)](../../hermes_cli/kanban_db.py#L38460) |
| 37408 | [Retention + garbage collection](../../hermes_cli/kanban_db.py#L38862) |
| 37457 | [Worker log accessor](../../hermes_cli/kanban_db.py#L38911) |
| 37506 | [Assignee enumeration (known profiles + per-profile board stats)](../../hermes_cli/kanban_db.py#L38960) |
| 37598 | [Runs (attempt history on a task)](../../hermes_cli/kanban_db.py#L39052) |
| 37646 | [Durable Kanban ↔ TMAX execution capsule](../../hermes_cli/kanban_db.py#L39100) |
