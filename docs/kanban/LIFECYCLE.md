...
| triage → todo | [`specify_triage_task`](../../hermes_cli/kanban_db.py#L19243) | CLI / sweep | row must still be in `triage` | specify returns false; status did not change |
...
| todo\|blocked → ready | [`promote_task`](../../hermes_cli/kanban_db.py#L18399) | CLI | typed wait or unfinished parents; blocked tasks cannot force past parents | command reports the refusing parent/wait |
...
| running\|ready → blocked | [`block_task`](../../hermes_cli/kanban_db.py#L18058) | worker / CLI / review gate | stale run ID, wrong status, invalid review-origin contract | block returns false; rejection event explains the mismatch |
| running\|ready → todo | [`block_task`](../../hermes_cli/kanban_db.py#L18058) | worker | a valid unsatisfied dependency wait deliberately parks on `todo` | `wait_registered` plus a `blocked` event whose status is `todo` |
| running\|ready → triage | [`block_task`](../../hermes_cli/kanban_db.py#L18058) | worker / sweep | same non-review block kind must reach [`BLOCK_RECURRENCE_LIMIT`](../../hermes_cli/kanban_db.py#L175) | `block_loop_detected` event; card is in `triage` |
| running\|ready\|blocked → review | [`_submit_for_review`](../../hermes_cli/kanban_db.py#L15538) | worker / review gate | workflow identity, worker gate, expected run CAS | task blocks on identity failure or completion raises a worker-gate error |
...
| running\|ready\|blocked → ready | [`_advance_workflow_step`](../../hermes_cli/kanban_db.py#L15902) | worker / workflow gate | no valid next step, stale run ID, wrong status | task completes normally or update returns false |
| running → review | [`_maybe_advance_review_chain`](../../hermes_cli/kanban_db.py#L16066) | review gate | verdict must be approved, workflow identity valid, another stage exists | identity failure blocks; final stage proceeds to `done` |
| running\|ready\|blocked → done | [`complete_task`](../../hermes_cli/kanban_db.py#L16382) | worker / CLI / review gate | hallucinated cards, worker/review gate, stale run, integration park | exception/event or card moves to `review`/`blocked` instead |
...
| blocked\|scheduled → todo\|ready | [`unblock_task`](../../hermes_cli/kanban_db.py#L19104) | CLI / automation | typed wait unless audited override; unfinished parents route to `todo` | unblock returns false or succeeds into `todo` |
| todo\|ready\|running\|blocked → scheduled | [`schedule_task`](../../hermes_cli/kanban_db.py#L21121) | CLI / cron / worker | active typed wait, stale expected run ID | schedule returns false; previous status remains |
| any non-archived → archived | [`archive_task`](../../hermes_cli/kanban_db.py#L20300) | CLI / sweep | dependent wait, changed ownership generation, surviving worker | archive refuses; wait conflict or `reclaim_deferred` is visible. Operator archives (CLI + dashboard, `retrigger_integration=True`) also re-run chain integration after a successful archive — synchronous gate+merge in the caller, see `maybe_retrigger_integration_after_archive` |

## Dispatch path: tick to worker process

1. [`dispatch_once`](../../hermes_cli/kanban_db.py#L29767) resolves the board DB
...
2. [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L29858) reaps zombies
   and pending continuations, refreshes Claude-CLI heartbeats through
   [`heartbeat_live_claude_cli_workers`](../../hermes_cli/kanban_db.py#L22310),
...
3. It selects unclaimed `ready` rows by priority and age. Before claim it applies
   ...
   contract, and [`check_respawn_guard`](../../hermes_cli/kanban_db.py#L28079).
   Advisory holds leave the row `ready`; [`summarize_dispatch_holds`](../../hermes_cli/kanban_db.py#L28319)
...
5. [`_resolve_dispatch_workspace`](../../hermes_cli/kanban_db.py#L21011)
   chooses managed worktree provisioning or existing-workspace resolution.
   [`resolve_workspace`](../../hermes_cli/kanban_db.py#L20906) defines the
...
6. A writable shared chain worktree acquires a writer lease. Then
   [`_default_spawn`](../../hermes_cli/kanban_db.py#L32277) freezes the worker
   environment and launch route, and [`_launch_worker_process`](../../hermes_cli/kanban_db.py#L31634)
...
2. [`_default_spawn`](../../hermes_cli/kanban_db.py#L32277) reads that stamp back
   via [`_persisted_spawn_identity`](../../hermes_cli/kanban_db.py#L32070) and
...
> [`_default_spawn`](../../hermes_cli/kanban_db.py#L32277) is exactly that — it
...
[`build_worker_context`](../../hermes_cli/kanban_db.py#L33752) returns the
canonical phase-aware bounded brief built by
[`render_worker_brief_for_task`](../../hermes_cli/kanban_db.py#L33713). It
...
[`_default_spawn`](../../hermes_cli/kanban_db.py#L32277) additionally pins the
...
[`heartbeat_worker`](../../hermes_cli/kanban_db.py#L22126); Claude-CLI lanes are
bridged by [`heartbeat_live_claude_cli_workers`](../../hermes_cli/kanban_db.py#L22310).
The daemon publishes board counts and tick health through
[`write_kanban_dispatcher_heartbeat`](../../hermes_cli/kanban_db.py#L28024).
...
[`complete_task`](../../hermes_cli/kanban_db.py#L16382) is a funnel, not a
...
3. **Token levers** — [`_deterministic_review_skip`](../../hermes_cli/kanban_db.py#L15358)
   and [`_tip_defer_review`](../../hermes_cli/kanban_db.py#L15400) may skip the
   LLM review; otherwise [`_submit_for_review`](../../hermes_cli/kanban_db.py#L15538)
...
5. **Stage advance** — [`_maybe_advance_review_chain`](../../hermes_cli/kanban_db.py#L16066)
...
[`integrate_chain`](../../hermes_cli/kanban_worktrees.py#L6230) is *the* single
...
|---|---|---|---|
| 0 | live-checkout precheck | [`_integrate_precheck_live`](../../hermes_cli/kanban_worktrees.py#L5898) | `MERGE_HEAD`/rebase in progress, or checked-out branch ≠ frozen merge target |
| 1 | artifact preservation | [`_preserve_or_park_chain_artifacts`](../../hermes_cli/kanban_worktrees.py#L5935) | chain worktree dirty with non-preservable files |
| 2 | nothing-to-merge | [`_integrate_empty_or_already_merged`](../../hermes_cli/kanban_worktrees.py#L5988) | handles `ahead == 0`: already-integrated, or replays a previously reverted merge |
| 3 | dirty overlap | inline in `integrate_chain` | a foreign dirty file in the live checkout overlaps the branch diff |
| 4 | rebase onto target | [`_integrate_rebase_branch`](../../hermes_cli/kanban_worktrees.py#L6116) | conflict → `rebase_conflict` (routed back to the coder, **not** a park) |
| 5 | merge + post-merge gate | [`_integrate_merge_and_gate`](../../hermes_cli/kanban_worktrees.py#L6166) | merge conflict → `merge --abort`; red gate → `revert -m 1` + park |

The post-merge gate runs at the exact merge commit inside a detached validation
worktree — [`_run_gate_in_validation_worktree`](../../hermes_cli/kanban_worktrees.py#L5539)
— never in the possibly-dirty live checkout. Every exception path there returns
`(False, …)` — it fails **closed**.

Gate selection is
[`_integration_gate_for_repo`](../../hermes_cli/kanban_worktrees.py#L318):
a per-repo command list from `kanban.integration_gate.repos` if configured, else
[`fo_integration_gate`](../../hermes_cli/kanban_worktrees.py#L5832) for the FO
repo, else [`default_quick_gate`](../../hermes_cli/kanban_worktrees.py#L5805).
...
A park becomes [`_park_integration`](../../hermes_cli/kanban_db.py#L17722),
...
[`maybe_integrate_on_complete`](../../hermes_cli/kanban_worktrees.py#L7764) only
integrates when this completion closes the **last open task** of a provisioned
chain. [`_find_open_chain_sibling`](../../hermes_cli/kanban_worktrees.py#L6955)
...
requires *positive* evidence: [`_run_worker_gate`](../../hermes_cli/kanban_db.py#L15265)
...
[`_capture_review_diff_snapshot`](../../hermes_cli/kanban_db.py#L15041) against
...
| Respawn cooldown/duplicate-work guard | recent rate limit/transient retry/auth failure/success, active PR, or invalid code contract | [`check_respawn_guard`](../../hermes_cli/kanban_db.py#L28079) | card remains `ready`; deduped `respawn_guarded` event names the reason | wait for cooldown, correct auth/contract, close or deliberately requeue after successful work, resolve the PR |
| G1 cumulative input-token runaway | all-run input sum exceeds configured per-task cap; one actionable review extension may be allowed | [`_park_budget_runaway`](../../hermes_cli/kanban_db.py#L25393) | card becomes capacity-blocked; `budget_runaway_parked` and operator-escalation events include sum/cap/run count | inspect the runaway, then operator unblocks/reassigns/closes; raising the cap alone does not change the parked state |
| Dispatch holds | repo, chain-worktree, writer lease, daily budget, role mismatch, or per-profile concurrency is occupied | [`summarize_dispatch_holds`](../../hermes_cli/kanban_db.py#L28319) | card stays `ready`; tick bucket/event names `repo_serialized`, `chain_worktree_serialized`, `worktree_writer_active`, `budget_held`, `role_fit_held`, or profile cap | let holder finish/reclaim, repair stale writer ownership, adjust routing/cap, or remove the conflicting work |
| Review ping-pong breaker | repeated review-origin `REQUEST_CHANGES` reaches configured maximum rounds | [`block_task`](../../hermes_cli/kanban_db.py#L18058) | task is `blocked` as `needs_input`; reason starts `review ping-pong breaker`; operator escalation class is `review_pingpong` | operator resolves findings and explicitly unblocks/respecs/reassigns; it does not auto-retry |
...
| Global/profile concurrency | live running count reaches board, spawn, or profile cap | [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L29858) | no spawn; profile-capped tasks appear in the hold result, global cap returns early | allow running tasks to terminate/reclaim or change the configured cap |
| Nonspawnable assignee | assigned name is not an on-disk Hermes profile | [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L29858) | `nonspawnable` event; unknown lanes also emit one operator escalation | assign a real profile or provision the intended profile; terminal pull-only lanes remain intentionally held |
| Workspace provisioning/base drift | invalid path, worktree lock/timeout, rebase conflict, or reused base differs | [`_resolve_dispatch_workspace`](../../hermes_cli/kanban_db.py#L21011) | claimed run is requeued/blocked; events include spawn retry/failure or `worker_base_rejected` | fix path/git state; transient timeouts back off; conflicts use the bounded fixer path or operator repair |
...
| Integration park | a precheck, the merge, or the post-merge gate failed | [`integrate_chain`](../../hermes_cli/kanban_worktrees.py#L6230) → [`_park_integration`](../../hermes_cli/kanban_db.py#L17722) | `integration_parked` event; closing run outcome is `integration_parked`, task `blocked` | fix the stated cause, then unblock; a red gate means the merge was already reverted |
| Rebase conflict | chain branch does not replay onto the live target | [`_integrate_rebase_branch`](../../hermes_cli/kanban_worktrees.py#L6116) | `integration_rebase_conflict`; rebase aborted, worktree back at its committed state | routed back to the coder as fixer work, deliberately NOT an operator park |
| Integration retry exhausted | a `transient`-classed park kept failing | [`_integration_park_class`](../../hermes_cli/kanban_worktrees.py#L431) | bounded `integration_retry` events; re-park once reclassified non-transient | resolve the underlying dirt/lock; the retry counter is separate from the failure breaker |
...
[`_record_task_failure`](../../hermes_cli/kanban_db.py#L24003);
[`_record_spawn_failure`](../../hermes_cli/kanban_db.py#L24411) delegates to it.
...
The post-merge [`default_quick_gate`](../../hermes_cli/kanban_worktrees.py#L5805)
enforces the same hold. Its compatibility mapper,
[`_affected_pytest_modules`](../../hermes_cli/kanban_worktrees.py#L4901), and
...
[`_park_integration`](../../hermes_cli/kanban_db.py#L17722) blocks via an UPDATE
...
| 16100 | [Workspace / tmux cleanup](../../hermes_cli/kanban_db.py#L16996) |
| 16579 | [First-use tip for scratch workspaces](../../hermes_cli/kanban_db.py#L17477) |
| 19637 | [Workspace resolution](../../hermes_cli/kanban_db.py#L20596) |
| 20188 | [Dispatcher (one-shot pass)](../../hermes_cli/kanban_db.py#L21188) |
| 20218 | [Respawn guard constants](../../hermes_cli/kanban_db.py#L21218) |
| 24185 | [G1: per-task cumulative input-token runaway guard](../../hermes_cli/kanban_db.py#L25187) |
| 27527 | [OpenClaw cross-system dispatch (Mission-Control via HMAC-signed envelopes)](../../hermes_cli/kanban_db.py#L28534) |
| 27764 | [B1a — tree-wide inventory/hygiene must not land on research/premium LLM loops](../../hermes_cli/kanban_db.py#L28771) |
| 31557 | [Long-lived dispatcher daemon](../../hermes_cli/kanban_db.py#L32598) |
| 31631 | [Worker context builder (what a spawned worker sees)](../../hermes_cli/kanban_db.py#L32672) |
| 32263 | [Scope-contract template expansion (PlanSpec B)](../../hermes_cli/kanban_db.py#L33304) |
| 32705 | [Stats + SLA helpers](../../hermes_cli/kanban_db.py#L33766) |
| 35429 | [Epics (N-E3) — durable goals spanning multiple task trees](../../hermes_cli/kanban_db.py#L36499) |
| 35603 | [Disposition Ledger (FRD-S1) — additive; no wiring into completion path yet](../../hermes_cli/kanban_db.py#L36673) |
| 36457 | [Lanes (night-sprint F1) — switchable profile→(runtime, model) presets](../../hermes_cli/kanban_db.py#L37587) |
| 36887 | [Notification subscriptions (used by the gateway kanban-notifier)](../../hermes_cli/kanban_db.py#L38025) |
| 37006 | [Browser Web Push subscriptions (used by the control dashboard)](../../hermes_cli/kanban_db.py#L38144) |
| 37408 | [Retention + garbage collection](../../hermes_cli/kanban_db.py#L38546) |
| 37457 | [Worker log accessor](../../hermes_cli/kanban_db.py#L38595) |
| 37506 | [Assignee enumeration (known profiles + per-profile board stats)](../../hermes_cli/kanban_db.py#L38644) |
| 37598 | [Runs (attempt history on a task)](../../hermes_cli/kanban_db.py#L38736) |
| 37646 | [Durable Kanban ↔ TMAX execution capsule](../../hermes_cli/kanban_db.py#L38784) |
