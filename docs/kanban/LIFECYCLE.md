...
| todo\|ready\|running\|blocked → scheduled | [`schedule_task`](../../hermes_cli/kanban_db.py#L20984) | CLI / cron / worker | active typed wait, stale expected run ID | schedule returns false; previous status remains |
...
1. [`dispatch_once`](../../hermes_cli/kanban_db.py#L29630) resolves the board DB
...
2. [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L29721) reaps zombies
   and pending continuations, refreshes Claude-CLI heartbeats through
   [`heartbeat_live_claude_cli_workers`](../../hermes_cli/kanban_db.py#L22173),
...
3. It selects unclaimed `ready` rows by priority and age. Before claim it applies
   ...
   contract, and [`check_respawn_guard`](../../hermes_cli/kanban_db.py#L27942).
   Advisory holds leave the row `ready`; [`summarize_dispatch_holds`](../../hermes_cli/kanban_db.py#L28182)
...
5. [`_resolve_dispatch_workspace`](../../hermes_cli/kanban_db.py#L20874)
...
6. A writable shared chain worktree acquires a writer lease. Then
   [`_default_spawn`](../../hermes_cli/kanban_db.py#L32124) freezes the worker
   environment and launch route, and [`_launch_worker_process`](../../hermes_cli/kanban_db.py#L31485)
...
2. [`_default_spawn`](../../hermes_cli/kanban_db.py#L32124) reads that stamp back
   via [`_persisted_spawn_identity`](../../hermes_cli/kanban_db.py#L31917) and
...
> [`_default_spawn`](../../hermes_cli/kanban_db.py#L32124) is exactly that — it
...
[`build_worker_context`](../../hermes_cli/kanban_db.py#L33581) returns the
canonical phase-aware bounded brief built by
[`render_worker_brief_for_task`](../../hermes_cli/kanban_db.py#L33542). It
...
[`_default_spawn`](../../hermes_cli/kanban_db.py#L32124) additionally pins the
...
[`heartbeat_worker`](../../hermes_cli/kanban_db.py#L21989); Claude-CLI lanes are
bridged by [`heartbeat_live_claude_cli_workers`](../../hermes_cli/kanban_db.py#L22173).
The daemon publishes board counts and tick health through
[`write_kanban_dispatcher_heartbeat`](../../hermes_cli/kanban_db.py#L27887).
...
| Respawn cooldown/duplicate-work guard | recent rate limit/transient retry/auth failure/success, active PR, or invalid code contract | [`check_respawn_guard`](../../hermes_cli/kanban_db.py#L27942) | card remains `ready`; deduped `respawn_guarded` event names the reason | wait for cooldown, correct auth/contract, close or deliberately requeue after successful work, resolve the PR |
| G1 cumulative input-token runaway | all-run input sum exceeds configured per-task cap; one actionable review extension may be allowed | [`_park_budget_runaway`](../../hermes_cli/kanban_db.py#L25256) | card becomes capacity-blocked; `budget_runaway_parked` and operator-escalation events include sum/cap/run count | inspect the runaway, then operator unblocks/reassigns/closes; raising the cap alone does not change the parked state |
| Dispatch holds | repo, chain-worktree, writer lease, daily budget, role mismatch, or per-profile concurrency is occupied | [`summarize_dispatch_holds`](../../hermes_cli/kanban_db.py#L28182) | card stays `ready`; tick bucket/event names `repo_serialized`, `chain_worktree_serialized`, `worktree_writer_active`, `budget_held`, `role_fit_held`, or profile cap | let holder finish/reclaim, repair stale writer ownership, adjust routing/cap, or remove the conflicting work |
...
| Global/profile concurrency | live running count reaches board, spawn, or profile cap | [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L29721) | no spawn; profile-capped tasks appear in the hold result, global cap returns early | allow running tasks to terminate/reclaim or change the configured cap |
| Nonspawnable assignee | assigned name is not an on-disk Hermes profile | [`_dispatch_once_locked`](../../hermes_cli/kanban_db.py#L29721) | `nonspawnable` event; unknown lanes also emit one operator escalation | assign a real profile or provision the intended profile; terminal pull-only lanes remain intentionally held |
| Workspace provisioning/base drift | invalid path, worktree lock/timeout, rebase conflict, or reused base differs | [`_resolve_dispatch_workspace`](../../hermes_cli/kanban_db.py#L20874) | claimed run is requeued/blocked; events include spawn retry/failure or `worker_base_rejected` | fix path/git state; transient timeouts back off; conflicts use the bounded fixer path or operator repair |
...
| Worker exits without terminal lifecycle call | subprocess exits zero while task is still running | [`detect_crashed_workers`](../../hermes_cli/kanban_db.py#L22693) | `protocol_violation` or `deliverable_posted_not_completed`; bounded repeats end in `gave_up` | recover posted evidence or rerun with the required complete/block call; operator unblocks after breaker trip |
| Spawn/config failure breaker | model route, executable, workspace, or repeated spawn fails | [`_record_spawn_failure`](../../hermes_cli/kanban_db.py#L24274) | failure counter and spawn events accumulate; terminal attempt becomes blocked/auto-blocked | repair deterministic config; allow bounded transient retry, then explicitly unblock after correction |
...
[`_record_task_failure`](../../hermes_cli/kanban_db.py#L23866);
[`_record_spawn_failure`](../../hermes_cli/kanban_db.py#L24274) delegates to it.
...
| 20188 | [Dispatcher (one-shot pass)](../../hermes_cli/kanban_db.py#L21051) |
| 20218 | [Respawn guard constants](../../hermes_cli/kanban_db.py#L21081) |
| 24185 | [G1: per-task cumulative input-token runaway guard](../../hermes_cli/kanban_db.py#L25050) |
| 27527 | [OpenClaw cross-system dispatch (Mission-Control via HMAC-signed envelopes)](../../hermes_cli/kanban_db.py#L28397) |
| 27764 | [B1a — tree-wide inventory/hygiene must not land on research/premium LLM loops](../../hermes_cli/kanban_db.py#L28634) |
| 31557 | [Long-lived dispatcher daemon](../../hermes_cli/kanban_db.py#L32445) |
| 31631 | [Worker context builder (what a spawned worker sees)](../../hermes_cli/kanban_db.py#L32519) |
| 32263 | [Scope-contract template expansion (PlanSpec B)](../../hermes_cli/kanban_db.py#L33151) |
| 32705 | [Stats + SLA helpers](../../hermes_cli/kanban_db.py#L33595) |
| 35429 | [Epics (N-E3) — durable goals spanning multiple task trees](../../hermes_cli/kanban_db.py#L36328) |
| 35603 | [Disposition Ledger (FRD-S1) — additive; no wiring into completion path yet](../../hermes_cli/kanban_db.py#L36502) |
| 36457 | [Lanes (night-sprint F1) — switchable profile→(runtime, model) presets](../../hermes_cli/kanban_db.py#L37416) |
| 36887 | [Notification subscriptions (used by the gateway kanban-notifier)](../../hermes_cli/kanban_db.py#L37854) |
| 37006 | [Browser Web Push subscriptions (used by the control dashboard)](../../hermes_cli/kanban_db.py#L37973) |
| 37408 | [Retention + garbage collection](../../hermes_cli/kanban_db.py#L38375) |
| 37457 | [Worker log accessor](../../hermes_cli/kanban_db.py#L38424) |
| 37506 | [Assignee enumeration (known profiles + per-profile board stats)](../../hermes_cli/kanban_db.py#L38473) |
| 37598 | [Runs (attempt history on a task)](../../hermes_cli/kanban_db.py#L38565) |
| 37646 | [Durable Kanban ↔ TMAX execution capsule](../../hermes_cli/kanban_db.py#L38613) |
