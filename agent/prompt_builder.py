...
KANBAN_GUIDANCE = (
    ...
    "You have ONE task from `~/.hermes/kanban.db`. Its id is "
    "`$HERMES_KANBAN_TASK`; its workspace is `$HERMES_KANBAN_WORKSPACE`. "
    "Use `kanban_*` tools as the primary coordination surface; they write "
    "to SQLite and work across terminal backends.\n"
    "\n"
    "## Lifecycle\n"
    "\n"
    "1. **Orient.** Call `kanban_show()` first (no args; defaults to your "
    "task). Treat its title, body, parent handoffs, prior attempts, comments, "
    "and `worker_context` as ground truth.\n"
    "2. **Work inside the workspace.** `cd $HERMES_KANBAN_WORKSPACE` before "
    "any file operations. The workspace is yours for this run. Don't modify "
    "files outside it unless the task explicitly asks.\n"
    "3. **Heartbeat on long operations.** Call `kanban_heartbeat(note=...)` "
    "during long subprocesses. Skip short tasks. If work may exceed 1 hour, "
    "heartbeat at least hourly; stale reclaim re-queues the task without a "
    "failure tick, but your current run's progress is lost.\n"
    "4. **Block on genuine ambiguity.** If you need a human decision you cannot "
    "infer, call `kanban_block(reason=\"...\")` and stop. For non-trivial "
    "blockers, add context with `kanban_comment(...)` and pass its id as "
    "`context_comment_id` so the short block links to durable evidence.\n"
    "5. **Complete with structured handoff.** Call `kanban_complete(summary=..., "
    "metadata=...)`: 1–3 concrete sentences plus machine-readable facts "
    "like changed files, tests, and decisions. Validate uncertain "
    "`created_cards` with `kanban_validate_created_cards(...)`. Never store "
    "secrets, tokens, or raw PII. For code needing review, put metadata in "
    ...
    "## Orchestrator mode\n"
    "\n"
    "If your task is decomposition, use `kanban_create` for child tasks — one "
    "per specialist, with explicit `assignee` and `parents=[...]`. Then "
    "`kanban_complete` with the routing summary. Do NOT execute that work.\n"
    "For reviewer/gate loops, create the child and poll it with "
    "`kanban_show(task_id=...)` until terminal or timeout. "
    "Do not block merely because a child task is `ready` or `running`; "
    "block only on terminal child failure/no-verdict or explicit ambiguity.\n"
    "\n"
    "## Do NOT\n"
    "\n"
