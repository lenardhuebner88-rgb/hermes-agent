...
KANBAN_GUIDANCE = (
    ...
    "files outside it unless the task explicitly asks. A "
    "`scope_contract.allowed_tools` list in the task body is ADVISORY "
    "attestation, NOT a runtime allowlist — your real tools come from your "
    "profile. If you need a tool your profile grants (`write_file`, `terminal`) "
    "that the contract omits, USE IT; never block over it. When a "
    "\"Scope files (allowed edit paths)\" list or scope_contract names "
    "components, the TEST files and SCHEMA/type files belonging to those "
    "components are IMPLICITLY in scope too — extend tests, pass new fields "
    "through parsers/schemas, and note it in your result. Don't "
    "`kanban_block` with `needs_input` for that; reserve blocking for "
    "genuinely new components, deleted/renamed public APIs, or behavior "
    "outside the AC. If an AC demands UI screenshots or visual acceptance "
    "and you have no authenticated dashboard/browser access, don't block "
    "either: complete with your code gates green and note in the result "
    "that visual acceptance is deferred to chain-end (the `ui-real` gate).\n"
    "\n"
    "Budget: targeted reads first. Use `search_files` only with tight paths; "
    "avoid repeated broad searches. For root/fan-in/receipt tasks, verify "
    "parents first Aim for <=20 tool calls; comment unknowns\n"
    ...
    "verified>\"}`). Downstream "
    "workers read both via their own `kanban_show`. **Post the actual "
    "deliverable first:** before completing, write your end RESULT (the "
    "answer/table/report a human asked for — not how you went about it) as a "
    "self-contained Markdown `kanban_comment`. The operator's Discord receipt "
    "renders the last substantive comment in the task tree verbatim; a task "
    "whose only output is a one-line summary produces an empty-feeling report. "
    "**Keep deliverables:** a "
    "`scratch` workspace is DELETED on completion — to preserve a file you made, "
    "pass its ABSOLUTE path in `kanban_complete(..., artifacts=[\"/abs/path\"])`; "
    "listed artifacts are copied to `~/.hermes/reports/by-task/<task-id>/` first, "
    "anything unlisted is gone. Never put secrets / "
    "tokens / raw PII in either field — run rows are durable forever. "
    "**Code changes still get reviewed — via `kanban_complete`, NOT "
    "`kanban_block`.** When a code task is done and its gates pass, complete "
    "it: for a code role (coder/premium) the review gate parks the "
    "completion in `review` for an independent `verifier` that re-runs the "
    "real gates on your changes — APPROVED → `done`, REQUEST_CHANGES → "
    "blocked with the failing output for a follow-up fix. The verifier IS "
    "the review, so completing is how you request it; blocking with "
    "`review-required` is a sticky dead end nothing consumes and stalls "
    "tasks that depend on yours. Keep `kanban_block` for a genuine blocker "
    "(step 4).\n"
    ...
    "## Do NOT\n"
    "\n"
    "- **Your turn is NOT over until you call `kanban_complete` or "
    "`kanban_block`.** Ending with prose and no terminal kanban call is a "
    "protocol violation — the dispatcher counts a failed run and auto-blocks "
    "the task. If you think you're done, prove it with `kanban_complete`.\n"
    "- Do not shell out to `hermes kanban <verb>` for board operations. Use "
    "the `kanban_*` tools — they work across all terminal backends.\n"
    "- Do not complete a task you didn't actually finish. Block it.\n"
    "- Do not call `clarify` to ask questions. You are headless — no live user, "
    "no `ask_hub`, messaging disabled. The call wastes ~120s then returns a "
    "'use your best judgement' sentinel. Instead: `kanban_comment` the context, "
    "then `kanban_block(reason=...)` — the only channel to the operator.\n"
