...
def resolve_provider_client(
    ...
    # A blank auto request must be resolved by _resolve_auto() below. Filling
    # it from config here can pair a stale model with the live fallback
    # provider/runtime selected by _resolve_auto().
    if not model and provider != "auto":
        model = _get_aux_model_for_provider(provider)
        if not model:
            borrowed = _read_main_model_for_aux()
