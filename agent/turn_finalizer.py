...
def _accepted_kanban_completion(task_id: str) -> tuple[bool, str | None]:
    """Return whether the current kanban worker already completed its run.

    A worker can spend its final allowed model turn on ``kanban_complete``.
    That tool mutates kanban.db immediately, then the agent loop exits with no
    follow-up prose turn because the iteration budget is exhausted. Treat the
    accepted DB transition as terminal for this chat turn instead of recording a
    synthetic budget failure over it.
    """
    if not task_id:
        return False, None
    try:
        ...
        try:
            ...
            run = _kb.latest_run(_conn, task_id)
        finally:
            try:
                _conn.close()
            except Exception:
    ...
    if task is None or run is None:
        return False, None
    if getattr(run, "outcome", None) != "completed":
        return False, None
    if getattr(task, "status", None) == "running":
        return False, None
    return True, getattr(run, "summary", None) or getattr(task, "result", None)
...
def finalize_turn(
    ...
    *,
    ...
    completed = (
        ...
        and (
            api_call_count < agent.max_iterations
            or _kanban_completion_accepted
    ...
    # Guarded like the cleanup steps above: this block is pure logging,
    # and it walks message/tool_calls shapes that can be malformed
    # (truncated persistence, foreign providers) — a raise here would
    # discard the already-computed final_response, the exact failure
    # the "never lose the response" contract above exists to prevent.
    try:
        ...
        if _last_msg_role == "tool":
            ...
            for _m in reversed(messages):
                ...
                if _m.get("role") == "assistant" and _m.get("tool_calls"):
                    _tcs = _m["tool_calls"]
                    if _tcs and isinstance(_tcs[-1], dict):
                        _fn = _tcs[-1].get("function")
                        if isinstance(_fn, dict):
                            _last_tool_name = _fn.get("name")
                    break
        ...
        logger.error("finalize_turn: turn-exit diagnostic log failed: %s", _diag_err, exc_info=True)
