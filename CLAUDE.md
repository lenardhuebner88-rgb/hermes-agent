...
- Android (`android/hermes-deck`): `scripts/gate-android.sh` — Lint + JVM unit tests; `--ui` adds instrumented Compose tests on an emulator it boots headless and shuts down itself. The SDK and the AVDs already live on the second disk (`~/Android/Sdk` is a bind mount on `/dev/sda1`); storage is not the constraint. Details and the three traps it encodes: `AGENTS.md` → *Hard pitfalls*.
