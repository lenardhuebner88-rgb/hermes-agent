...
def _materialized_scores(conn: sqlite3.Connection) -> dict[str, dict[str, object]]:
    ...
    Numeric and binary signals are arithmetic means.  Categorical signals are
    value-frequency maps so values such as task states are never coerced into
    misleading numbers.  Every supported name is present even if it has no
    rows yet.
    """
    scores: dict[str, dict[str, object]] = {
        name: {"value": None, "count": 0}
        for name in (*_NUMERIC_MATERIALIZED_SCORE_NAMES, *_CATEGORICAL_MATERIALIZED_SCORE_NAMES)
    }
    numeric_placeholders = ", ".join("?" for _ in _NUMERIC_MATERIALIZED_SCORE_NAMES)
    numeric_rows = conn.execute(
        f"SELECT name, AVG(CAST(value AS REAL)) AS value, COUNT(*) AS count "
    ...
    for row in categorical_rows:
        ...
        values[str(row["value"])] = int(row["count"])
