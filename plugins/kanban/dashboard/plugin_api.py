        ...
        if payload.status is not None:
            ...
            if s == "done":
                ...
                ok = kanban_db.archive_task(
                    conn, task_id, retrigger_integration=True,
                )
            elif s == "running":
                raise HTTPException(
                    status_code=400,
            ...
            try:
                ...
                if payload.archive:
                    if not kanban_db.archive_task(
                        conn, tid, retrigger_integration=True,
                    ):
...
load_api_extension(Path(__file__).with_name("usage_consumption_routes.py"), globals(), extension_name="usage_consumption_routes")
load_api_extension(Path(__file__).with_name("hermes_observability_routes.py"), globals(), extension_name="hermes_observability_routes")
...
_digest_routes_module = load_api_extension(
    Path(__file__).with_name("digest_routes.py"),
    globals(),
    extension_name="digest_routes",
)
for _extension_name in _digest_routes_module.__all__:
    globals()[_extension_name] = getattr(_digest_routes_module, _extension_name)
