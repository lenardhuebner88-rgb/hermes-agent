...
def test_dispatch_holds_endpoint_repo_serialized(client, tmp_path):
    """GET /dispatch/holds returns the same per-task hold structure as the CLI."""
    repo = tmp_path / "holds_repo"
    repo.mkdir()
    ...
    r = client.get("/api/plugins/kanban/dispatch/holds")
    assert r.status_code == 200, r.text
    data = r.json()
    assert data["count"] == 1
