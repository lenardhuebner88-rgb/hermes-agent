...
def test_worktree_writer_lease_serializes_and_reaps_after_group_exit(
    ...
    monkeypatch.setattr(kb, "_pid_alive", lambda _pid: pid_alive)
    monkeypatch.setattr(kb, "_attach_or_reap_spawned_worker", lambda *_args: True)
    monkeypatch.setattr(
        kb,
    ...
    with kb.connect_closing() as conn:
        first = kb.create_task(
            conn, title="first writer", assignee="coder", kind="code",
            workspace_kind="dir", workspace_path=str(repo), priority=10,
        )
        second = kb.create_task(
            conn, title="second writer", assignee="coder", kind="code",
            workspace_kind="dir", workspace_path=str(repo),
        )

        identities = (
            kb._worktree_writer_lease_identity(conn, kb.get_task(conn, first)),
            kb._worktree_writer_lease_identity(conn, kb.get_task(conn, second)),
        )
        assert identities[0][0] == identities[1][0], identities
        result = kb.dispatch_once(conn, spawn_fn=fake_spawn, serialize_by_repo=False)
        assert spawned == [first]
        assert result.skipped_worktree_writer_active == [
            (second, "worktree_writer_active", first)
        ]
        assert kb.get_task(conn, second).status == "ready"
        assert conn.execute("SELECT task_id FROM worktree_writer_leases").fetchone()[0] == first

        run_id = kb.get_task(conn, first).current_run_id
...
def test_read_only_task_bypasses_active_worktree_writer_lease(
    ...
    def fake_spawn(task, _workspace):
        spawned.append(task.id)
        return 515151 + len(spawned)

    with kb.connect_closing() as conn:
        writer = kb.create_task(
            conn, title="writer", assignee="coder", kind="code", priority=10,
            workspace_kind="dir", workspace_path=str(workspace),
        )
        reader = kb.create_task(
            conn, title="reader", assignee="researcher-a", kind="research",
            workspace_kind="dir", workspace_path=str(workspace),
        )
        result = kb.dispatch_once(conn, spawn_fn=fake_spawn, serialize_by_repo=False)

        assert spawned == [writer, reader]
        assert result.skipped_worktree_writer_active == []
