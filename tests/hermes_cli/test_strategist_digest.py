...
def test_write_quarantines_harvest_noise_from_planspec_cluster(digest_override):
    """Even a stale/manual digest cannot turn classified noise into lever material."""
    digest_override.parent.mkdir(parents=True, exist_ok=True)
    (digest_override.parent / "harvest_candidates.json").write_text(
        json.dumps(
            {
                "noise_candidates": [
                    {
                        ...
                        "kind": "risk",
                        "source_severity": "real-risk",
                        "triage_severity": "real-risk",
                        "noise_class": "transient",
                        "noise_reason": "not reproduced after a one-off outcome",
                    },
                    {
                        "suggested_key": "disposition-di_confounded",
                        "kind": "follow_up",
                        "source_severity": "scope-note",
                        "triage_severity": "scope-note",
                        "noise_class": "confounded",
                        "noise_reason": "verification depends on merge/restart",
                    },
                ]
            }
        ),
    ...
    payload = {
        "clusters": [
            {
                "theme": "mixed",
                "item_ids": ["disposition-di_transient", "disposition-di_actionable"],
                "triage_severity": "real-risk",
                "recommendation": "planspec",
                "planspec_key": "disposition-di_transient",
            },
            {
                "theme": "only deferred verification",
                "item_ids": ["disposition-di_confounded"],
                "triage_severity": "scope-note",
                "recommendation": "planspec",
                "planspec_key": "disposition-di_confounded",
            },
        ],
        "left": [],
    }
    ...
    assert [entry["item_id"] for entry in data["noise"]] == [
        "disposition-di_transient",
        "disposition-di_confounded",
    ]
    assert data["clusters"] == [
        {
            "theme": "mixed",
            "item_ids": ["disposition-di_actionable"],
            "kind": "cluster",
            "source_severity": "none",
            "triage_severity": "real-risk",
            "severity": "real-risk",
