...
@pytest.mark.parametrize(
    ...
    [
        ("upload", "auth.json"),
        ("upload_stream", "mcp-tokens/server.json"),
        ("mkdir", "pairing"),
        ("delete", "config.yaml"),
    ],
)
def test_sensitive_managed_paths_are_write_denied(
    forced_files_client, method, path
):
    """The Files API must not mutate credential stores under HERMES_HOME."""
    client, root = forced_files_client
    root.mkdir(parents=True, exist_ok=True)
    target = root / path
    if method == "delete":
    ...
    if method == "upload":
    ...
    elif method == "upload_stream":
        response = client.post(
            "/api/files/upload-stream",
            data={"path": str(target), "overwrite": "true"},
            files={"file": ("server.json", b"secret", "application/json")},
        )
    elif method == "mkdir":
        ...
        response = client.request(
            "DELETE",
            "/api/files",
            json={"path": str(target), "recursive": False},
        )
...
def test_json_upload_rejects_dangling_leaf_symlink(forced_files_client, tmp_path):
    """A dangling leaf symlink must not escape the locked managed root."""
    client, root = forced_files_client
    root.mkdir(parents=True, exist_ok=True)
    outside = tmp_path / "outside"
    outside.mkdir()
    outside_target = outside / "auth.json"
    link = root / "benign-note.txt"
    try:
