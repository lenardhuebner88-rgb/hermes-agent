...
def _context_prompt(agent):
    # Fork helper kept next to upstream's _prompt_parts: both call sites exist
    # in this file (the fork's context-only assertions and upstream's
    # parts-dict assertions), so taking one side would break the other.
    return _prompt_parts(agent)["context"]
